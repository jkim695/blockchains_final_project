...
class TestDelete:
    ...
    def test_delete_non_existing_key(self):
        ...
        # Deleting a non-existing key should not change the root hash.
...
class TestRootHash:
    def test_determinism_same_ops_same_root(self):
        tree1 = MerklePrefixTree()
        tree2 = MerklePrefixTree()
        for k in [10, 20, 30]:
...
class TestGenerateProof:
    ...
    def test_proof_length_olog_n(self):
        """Insert 1000 keys; proof length should be <= 64 (KEY_BITS)."""
        tree = MerklePrefixTree()
        for i in range(1000):
            tree.insert(i, b"v")
